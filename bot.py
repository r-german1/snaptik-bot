...
async def start_command_handler(client, message: Message):
    welcome_text = (
        ...
        "🤖 ئەڤە مەزنترین و پێشکەفتنترین بۆتا داونلۆدکرنێ یە ل جیهانێ، کو ب هێزا 8000 سێرڤەران کار دکەت و کوالیتیا 4K و MP3 بێ کێشە بۆ تە دئینە خوارێ.\n\n"
        "👑 خودان و دامەزرێنەرێ ڕەها یێ ئەڤێ بۆتێ: @YUSEEF_SURCHI\n\n"
        "🔗 بۆ دەستپێکرنێ، لینکێ ڤیدیۆیا خۆ (تیکتۆک، اینستاگرام، یوتیوب، سناپچات) بۆ من بنێرە بۆ داونلۆدکرنێ!"
    )
...
async def downloader_core_handler(client, message: Message):
    ...
    process_msg = await message.reply_text(
        "⚡️ سیستەمێ ئینفینیتی (ب هێزا 8000 سێرڤەران): نوکە زانیاریێن ڤیدیۆیێ دئینم خوارێ...\n\n"
        "👑 خودان: @YUSEEF_SURCHI"
    )
